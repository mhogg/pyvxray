...
class PyvXRAYDB(AFXDataDialog):
    ...
    def __init__(self, form):
        ...
        if len(self.form.odbList)>0:
            for odbName in self.form.odbList:
                ComboBox_1.appendItem(odbName)
            self.form.odbNameKw.setValue(self.form.odbList[0])
            ComboBox_1.setMaxVisible(10)
            self.odbName = self.form.odbList[0]
        ...
        self.ComboBox_2a = AFXComboBox(p=VAligner_2, ncols=35, nvis=1, text='%-20s'%'Density variable:', tgt=form.BMDfonameKw, sel=0, pt=5, pb=5)    
        self.ComboBox_2a.setMaxVisible(10)      
        self.populateScalarListComboBox() 
        
        
        GroupBox_2a = FXGroupBox(p=TabItem_1, text='Implant region', opts=FRAME_GROOVE|LAYOUT_FILL_X|LAYOUT_FILL_Y)        
        self.cb1   = FXCheckButton(p=GroupBox_2a, text='Show implant on x-rays', tgt=form.showImplantKw, sel=0)        
        VAligner_2a = AFXVerticalAligner(p=GroupBox_2a, opts=0, x=0, y=0, w=0, h=0, pl=0, pr=0, pt=0, pb=0)        
        self.ComboBox_3 = AFXComboBox(p=VAligner_2a, ncols=35, nvis=1, text='Implant set:', tgt=form.iSetNameKw, sel=0, pt=5, pb=5)    
        self.ComboBox_3.setMaxVisible(10)      
        self.populateElementListComboBoxImplant()

        self.tf1   = AFXTextField(p=VAligner_2a, ncols=0, labelText='Density (kg/m^3):', tgt=form.iDensityKw, sel=0, w=331, opts=LAYOUT_FIX_WIDTH)
        ...
        AFXTextField(p=VAligner_3, ncols=26, labelText='Step list:', tgt=form.stepListKw, sel=0, pt=5, pb=5)
        form.stepListKw.setValue(', '.join(form.stepList))
        
        ComboBox_4 = AFXComboBox(p=VAligner_3, ncols=24, nvis=1, text='Coordinate system:', tgt=form.csysNameKw, sel=0, pt=5, pb=5)
        csyses = []
        ...
        for csys in csyses:
            ComboBox_4.appendItem(text=csys)
        ComboBox_4.setMaxVisible(5)
                
        AFXTextField(p=VAligner_3, ncols=26, labelText='%-30s'%'Mapping resolution (mm):', tgt=form.resGridKw, sel=0, pt=5, pb=5)
        ...
        AFXTextField(p=VAligner_4, ncols=18, labelText='Base name of xray file(s):', tgt=form.imageNameBaseKw, sel=0, pt=5, pb=5)
        AFXTextField(p=VAligner_4, ncols=18, labelText='Approx size of x-ray images (in pixels):  ', tgt=form.preferredXraySizeKw, sel=0, pt=5, pb=5)
        ComboBox_2 = AFXComboBox(p=VAligner_4, ncols=16, nvis=1, text='Image file format', tgt=form.imageFormatKw, sel=0, pt=5, pb=5)
    ...
    def populateElementListComboBoxImplant(self):
        ...
        for elementSet in self.form.elementSets:
            self.ComboBox_3.appendItem(elementSet)
    ...
    def populateScalarListComboBox(self):
        ...
        self.ComboBox_2a.clearItems()
        for scalar in self.form.scalarList:
            self.ComboBox_2a.appendItem(scalar)
